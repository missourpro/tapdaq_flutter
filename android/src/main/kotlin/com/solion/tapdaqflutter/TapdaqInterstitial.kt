...
class TapdaqInterstitial(private val registrar: PluginRegistry.Registrar,
  ...
  override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
    when(call.method) {
      "load" -> {
        ...
        Tapdaq.getInstance().loadInterstitial(registrar.activity(), adTag,
                TapdaqInterstitialListener(channel))
        result.success(null)
      }
      "isLoaded" -> {
        val adTag = call.argument<String>("adTag")
        result.success(Tapdaq.getInstance().isInterstitialReady(
                registrar.activity(), adTag))
      }
      "show" -> {
        val adTag = call.argument<String>("adTag")
        Tapdaq.getInstance().showInterstitial(registrar.activity(), adTag,
                TapdaqInterstitialListener(channel));
        result.success(null)
      }
